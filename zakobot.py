        ...
        if file.endswith(".py") and not file.startswith("_"):
            ...
            for cmd in categoryImport.commandList():
                try:
                    ...
                    text.warn(f"Command {cmd.name} is broken: {e}. Skipping...")
                    continue
...
You must be new to Discord bots, huh..."""
...
async def on_member_join(member):
    try:
        ...
        text.log(f"{member.name} ({member.id}) has joined {guild.name}")
...
async def on_member_remove(member):
    try:
        ...
        text.log(f"{member.name} ({member.id}) has left {guild.name}")
            ...
            for server in bot.guilds:
                ...
                if announceChannel != None and isGetUpdates and ((local['branch'] == "Developer" and zakostable not in guild.members) or local['branch'] == "Main"):
                    ...
                    if source == "self": #send message as Zako
                        for i in ["\n".join(announce.split('\n')[start:start+15]) for start in range(0, len(announce.split('\n')), 15)]:
